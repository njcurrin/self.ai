...
## Out of Scope
- Model conversion, quantization, LoRA merging (see cavekit-llamolotl-pipeline.md)
- Inference server management (see cavekit-llamolotl-inference.md)
- Node-based training pipeline UI (future investigation)
- W&B integration (optional, leave existing env var passthrough)

## Cross-References
- See also: cavekit-llamolotl-pipeline.md (pipeline consumes training outputs)
- See also: cavekit-llamolotl-inference.md (inference loads trained adapters)
- See also: cavekit-llamolotl-platform.md (logging/error handling spans training code)
