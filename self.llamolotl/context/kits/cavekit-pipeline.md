...
- [ ] When training completes, queued pipeline tasks start automatically (batch conversion)
**Dependencies:** none

## Out of Scope
- Dataset curation (see self.curator)
- Training job execution (see cavekit-training.md)
- Inference model loading (see cavekit-inference.md — but pipeline produces what inference loads)
- Node-based pipeline UI (future investigation)

## Cross-References
- See also: cavekit-training.md (training produces LoRA adapters that pipeline transforms)
- See also: cavekit-inference.md (inference loads GGUF models and adapters that pipeline produces)
- See also: cavekit-platform.md (logging standards, error handling patterns)
